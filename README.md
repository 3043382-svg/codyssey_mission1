...
**OrbStack을 사용한 이유:** 서울캠퍼스 환경은 시스템 보안 정책상 `sudo` 권한 사용이 제한됩니다. Docker Desktop을 직접 설치하거나 데몬을 제어할 수 없으므로, 별도 관리자 권한 없이 Docker 엔진을 구동하는 OrbStack을 사용했습니다. OrbStack 실행 시 내부적으로 Docker 엔진이 함께 구동되며, 터미널에서는 `docker` 명령을 동일하게 사용할 수 있습니다.
...
### #3 sudo 제약으로 Docker Desktop 설치 불가

- **문제** — 일반적인 방식으로 Docker를 설치하고 데몬을 제어하려 했으나 권한이 거부되었습니다.
- **원인 가설** — 캠퍼스 보안 정책으로 `sudo` 사용이 제한된 것으로 판단했습니다.
- **확인** — 관리자 권한이 필요한 설치 단계에서 반복적으로 차단되었습니다.
- **해결 / 대안** — 관리자 권한 없이 컨테이너 엔진을 구동하는 OrbStack을 사용했습니다. `docker version` 출력의 `Context: orbstack`으로 정상 연결을 확인했으며, 이후 `docker` 명령은 기존과 동일하게 사용했습니다.

### #4 OrbStack이 꺼져 있어 Docker 데몬 연결 실패
...
### #5 바인드 마운트 경로 오타로 인한 컨테이너 이름 충돌
...
### #6 `ubuntu:latest` 이미지가 로컬에 없어 실행이 지연됨
...
- Git 사용자 이름과 이메일은 `git config --global --list` 캡처에서 마스킹했습니다.
- Personal Access Token은 문서·로그·스크린샷 어디에도 기록하지 않았습니다.
- 스크린샷에 토큰, 비밀번호, 개인키, 인증 코드가 포함되지 않았음을 제출 전 전수 확인했습니다.
- 민감정보 노출이 확인될 경우 문서에서 즉시 제거하고 해당 자격증명을 재발급하는 것을 원칙으로 합니다.

---

## 8. 재현 시 유의사항

- 컨테이너 런타임은 OrbStack을 전제로 합니다. Docker Desktop 환경에서도 `docker` 명령은 동일하게 동작합니다.
- 바인드 마운트 명령의 `$(pwd)`는 실행 위치에 따라 달라집니다. 저장소 루트에서 실행하십시오.
- 호스트 포트 8080·8081이 이미 사용 중이면 다른 포트로 바꿔 실행하십시오. 컨테이너 내부 포트(80)는 그대로 둡니다.
- 실습 후 정리: `docker rm -f my-web-server bind-test vol-test2` / `docker volume rm mydata`
